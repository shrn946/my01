...
const RichEditor = dynamic(() => import("@/components/rich-editor").then(mod => mod.RichEditor), {
  ssr: false,
  loading: () => <div className="animate-pulse h-32 w-full bg-muted rounded-md" />
});
const MediaUploader = dynamic(() => import("@/components/media-uploader").then(mod => mod.MediaUploader), {
  ssr: false,
  loading: () => <div className="animate-pulse h-32 w-full bg-muted rounded-md" />
});
...
export default function DashboardClient({
  ...
  const [recentLeads, setRecentLeads] = useState<any[]>(initialRecentLeads || []);
  const [recentFinderLeads, setRecentFinderLeads] = useState<any[]>(initialFinderLeads || []);
