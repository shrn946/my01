...
import { deleteBlogPost, deleteHeroSlide, deleteMedia, deleteProject, deleteReview, deleteUser, saveBlogPost, saveHeroSlide, saveInnerHeroSettings, saveProject, saveReview, saveUser, uploadMedia, deletePortfolioExample, savePortfolioExample } from "@/lib/actions";
          ...
          <AdminTabs
            tabs={[
              ...
              },
              {
                ...
                children: (
                  <div className="max-w-4xl mx-auto space-y-12">
                    <AdminBlock title="Email Integration Test">
                      <TestEmailButton />
                    </AdminBlock>
              ...
              },
              {
                ...
                children: (
                  <div className="space-y-12">
                    <AdminBlock title="Add Portfolio Example">
                      <PortfolioExampleForm media={media} />
                    </AdminBlock>
                    <AdminBlock title="Manage Examples">
                      <div className="grid gap-6">
                        {portfolioExamples.map((example: any) => (
                          <EditableCard key={example.id} title={`${example.title} (${example.category})`} deleteAction={deletePortfolioExample} id={example.id}>
                            <PortfolioExampleForm example={example} media={media} />
                          </EditableCard>
                        ))}
                        {!portfolioExamples.length ? <Empty label="No portfolio examples found." /> : null}
                      </div>
                    </AdminBlock>
                  </div>
                )
...
function PortfolioExampleForm({ example, media }: { example?: any; media: any[] }) {
  const categories = [
    "Dentist",
    "Lawyer",
    "Restaurant",
  ...
  return (
    <form action={savePortfolioExample} className="grid gap-6 rounded-[2rem] border border-black/5 bg-white p-8 shadow-premium lg:p-10">
      <input type="hidden" name="id" value={example?.id ?? ""} />
      <div className="grid gap-6 md:grid-cols-2">
        <BaseInput label="Title" name="title" defaultValue={example?.title} required />
        <label className="grid gap-3 text-sm font-bold text-ink">
          Category
          <select name="category" defaultValue={example?.category ?? "Local Business"} className="w-full appearance-none rounded-2xl border border-slate-100 bg-white px-5 py-4 text-sm font-medium outline-none transition-all focus:border-primary focus:ring-4 focus:ring-primary/5">
            {categories.map(cat => (
      ...
      <BaseInput label="Website URL" name="url" defaultValue={example?.url} required />
      <MediaImageField label="Thumbnail Image URL" name="thumbnail" defaultValue={example?.thumbnail ?? ""} media={media} required />
      <TextArea label="Short Description" name="description" defaultValue={example?.description} required />
      <button className="flex w-full items-center justify-center gap-2 rounded-2xl bg-primary py-5 text-sm font-black text-white shadow-lg shadow-primary/20 transition-all hover:bg-blue-600">
        Save Portfolio Example
      </button>
    </form>
  );
}
